...
class BlogPost(models.Model):
    ...
    tag = models.CharField(
        max_length=15, choices=TAG_CHOICES, null=False, blank=False, default="SPORT"
    )
...
class Comment(models.Model):
    ...
    blog_post = models.ForeignKey(
        BlogPost, null=False, blank=False, on_delete=models.CASCADE
    )
...
class CommentLike(models.Model):
    comment = models.ForeignKey(
        Comment, null=False, blank=False, on_delete=models.CASCADE
    )
    liker = models.ForeignKey(
        settings.AUTH_USER_MODEL, null=False, blank=False, on_delete=models.CASCADE
    )
