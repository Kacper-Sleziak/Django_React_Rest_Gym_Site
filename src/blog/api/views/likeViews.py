...
def like_blog_post_view(request):
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            ...
            if serializer.errors:
                return Response(serializer.errors, stauts=status.HTTP_226_IM_USED)
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            ...
            queryset = BlogLike.objects.filter(liker=liker, blog_post=blog_post)
            
            if queryset.exists():
                return Response(BlogLikeSerializer(request.data), status=status.HTTP_200_OK)
...
def like_comment_view(request):
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            ...
            if queryset.exists():
                return Response(CommentLikeSerializer(request.data), status=status.HTTP_200_OK)
            return Response(status=status.HTTP_404_NOT_FOUND)
        return Response(stauts=status.HTTP_400_BAD_REQUEST)
